...
class ABC extends Thread{
  ...
  public ABC(PrintResult printer, int[][][] problem, 
               int employedSize, int onlookerSize, 
                int maxCycle, int penaltyType){
      ...
      for(int count=0; count<problem.length; count++){
        if(problem[ctr][count][1]==1){
          ...
          for(int ctr2=0; ctr2<subgrid.length; ctr2++){
            if (subgrid[ctr2].isBelong(
                  emptyCell[maxEmptyCell][1],
                    emptyCell[maxEmptyCell][0])) {
