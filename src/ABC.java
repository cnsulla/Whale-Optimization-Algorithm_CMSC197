...
class ABC extends Thread{
  ...
  private int penalty;
	ABC(PrintResult printer, int[][][] problem,int employedSize, int onlookerSize, int maxCycle, int penaltyType){
    ...
    System.out.println("============================================");
    if (penalty == 0)
      System.out.println("ABC Algorithm penalty type: Missing Numbers");
    else if (penalty == 1)
      System.out.println("ABC Algorithm penalty type: Sum-Product");
    else if (penalty == 2)
      System.out.println("ABC Algorithm penalty type: Sum-Product with no intrinsic subgrid constraint");
    System.out.println("ABC Algorithm started at time:\t" + timestart);
    
		//actual
		for(cycle=0; cycle<maxCycle && maxFit!=1; cycle++){
      //System.out.println(cycle + "/" + maxCycle);
			sumFitness=0;
			...
			if(scoutSize>0 && maxFit!=1){
				...
				for(int i=0; i<scoutSize && maxFit!=1; i++){
					v=new Bee(getProblemCopy(), subgrid, penalty);											//generating of new Solution
    ...
    System.out.println("ABC Algorithm started at time:\t" + timeend);
    long elapsed = timeend - timestart;
    System.out.println("ABC Algorithm finished after time (milliseconds):\t" + elapsed);
    System.out.println("ABC Algorithm finished after cycles:\t" + cycle);
    System.out.println("ABC Algorithm finished with fitness:\t" + bestBee.getFitness());
    System.out.println("============================================");
	...
	private void initialization(){
		...
		bestBee=new Bee(subgrid, penalty);
    System.out.println(penalty);
		for(int ctr=0; ctr<employedSize; ctr++){
			bee[ctr]=new Bee(getProblemCopy(), subgrid, penalty);
	...
	private Bee neighborhoodSearch(int i){
		...
		Bee newBee=new Bee(subgrid, penalty);
